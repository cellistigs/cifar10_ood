...
class Test_CINIC10:
	...
	def test_data_labels(self,tmpdir):
		"""Should unzip to the right folder if it doesn't exist. 
		...
		downloads.mkdir()
		downloads = os.path.join(here,"../data/cinic-10") 
		cd = CINIC10(Path(downloads),"test",labels=np.load("/home/ubuntu/ensemble_attention/scripts/outputs/2022-07-22/16-37-55/ood_cinic_preds.npy"),preload = True)
		sample = next(iter(cd))
